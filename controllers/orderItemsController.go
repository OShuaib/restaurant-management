...
func GetOrderItemsByOrder() gin.HandlerFunc{
	return func (c *gin.Context){
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error":"error occured while listing order item by order"})
			return
		}
		c.JSON(http.StatusOK, allOrderItems)
	}
...
func ItemsByOder(id string)(OrderItems []primitive.M, err error) {
	...
	unwindOrderStage := bson.D{{"$unwind", bson.D{{"path", "$order"},{"preserveNullAndEmptys", true}}}}
...
func UpdateOrderItem() gin.HandlerFunc{
	return func (c *gin.Context){
		...
		if orderItem.Quatity != nil {
			updateObj = append(updateObj, bson.E{"quantity",orderItem.Quatity})
		}
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error":"Order itrm update failed"})
			return
		}
		defer cancel()
...
func CreateOrderItem() gin.HandlerFunc{
	return func (c *gin.Context){
		...
		result, err := orderItemCollection.InsertOne(ctx, orderItemsToBeInserted)
